...
func handshake(network, address string, timeout time.Duration, ciphers []ciphersuite.CipherSuite) (TLS10, error) {

	conn, err := net.DialTimeout(network, address, timeout)
	if err != nil {
		return TLS10{}, fmt.Errorf("failed to connect to %s: %s", address, err)
	}
	defer conn.Close()
...
func Scan(network, address string, timeout time.Duration) (TLS10, error) {
